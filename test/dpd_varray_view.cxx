...
TEST(dpd_varray_view, constructor)
{
    ...
    dpd_varray<double> v0(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, layouts[0]);

    dpd_varray_view<double> v1;
    CHECK_DPD_MARRAY_VIEW_RESET(v1)

    for (unsigned j = 0;j < 6;j++)
    {
        dpd_varray_view<double> v2(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[j]);
        CHECK_DPD_MARRAY_VIEW(v2, data, j)
    }

    dpd_varray_view<double> v3(1, 2, array<array<char,2>,4>{{{3, 1}, {2, 2}, {1, 2}, {3, 4}}}, data, layouts[0]);
    CHECK_DPD_MARRAY_VIEW(v3, data, 0)

    dpd_varray_view<double> v5(v3);
    CHECK_DPD_MARRAY_VIEW(v5, data, 0)

    dpd_varray_view<double> v51(v0);
    CHECK_DPD_MARRAY_VIEW(v51, v0.data(), 0)

    dpd_varray_view<double> v6(dpd_varray_view<double>(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]));
    CHECK_DPD_MARRAY_VIEW(v6, data, 0)

    dpd_varray_view<const double> v9;
    CHECK_DPD_MARRAY_VIEW_RESET(v9)

    for (unsigned j = 0;j < 6;j++)
    {
        dpd_varray_view<const double> v10(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[j]);
        CHECK_DPD_MARRAY_VIEW(v10, data, j)
    }

    dpd_varray_view<const double> v11(1, 2, array<array<char,2>,4>{{{3, 1}, {2, 2}, {1, 2}, {3, 4}}}, data, layouts[0]);
    CHECK_DPD_MARRAY_VIEW(v11, data, 0)

    dpd_varray_view<const double> v13(v3);
    CHECK_DPD_MARRAY_VIEW(v13, data, 0)

    dpd_varray_view<const double> v131(v0);
    CHECK_DPD_MARRAY_VIEW(v131, v0.data(), 0)

    dpd_varray_view<const double> v14(v11);
    CHECK_DPD_MARRAY_VIEW(v14, data, 0)

    dpd_varray_view<const double> v15(dpd_varray_view<double>(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]));
    CHECK_DPD_MARRAY_VIEW(v15, data, 0)

    dpd_varray_view<const double> v16(dpd_varray_view<const double>(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]));
...
TEST(dpd_varray_view, reset)
{
    ...
    dpd_varray_view<double> v1;
    dpd_varray_view<const double> v2;
    dpd_varray_view<double> v3(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]);
    dpd_varray_view<const double> v4(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]);
    dpd_varray<double> v0(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, layouts[0]);
    ...
    v1.reset(1, 2, array<array<char,2>,4>{{{3, 1}, {2, 2}, {1, 2}, {3, 4}}}, data, layouts[0]);
    ...
    v1.reset(dpd_varray_view<double>(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]));
    CHECK_DPD_MARRAY_VIEW(v1, data, 0)
    ...
    v2.reset(1, 2, array<array<char,2>,4>{{{3, 1}, {2, 2}, {1, 2}, {3, 4}}}, data, layouts[0]);
    ...
    v2.reset(dpd_varray_view<double>(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]));
    CHECK_DPD_MARRAY_VIEW(v2, data, 0)

    v2.reset(dpd_varray_view<const double>(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[0]));
    CHECK_DPD_MARRAY_VIEW(v2, data, 0)
...
TEST(dpd_varray_view, permute)
{
    ...
    for (unsigned j = 0;j < 6;j++)
    {
        ...
        dpd_varray_view<double> v1(1, 2, {{3, 1}, {2, 2}, {1, 2}, {3, 4}}, data, layouts[j]);
        ...
        v1.permute(std::vector<char>{2, 0, 3, 1});
        EXPECT_EQ(data, v1.data());
        EXPECT_EQ(1u, v1.irrep());
        EXPECT_EQ(2u, v1.num_irreps());
...
TEST(dpd_varray_view, swap)
{
    ...
    dpd_varray_view<double> v1(1, 2, {{2, 3}, {2, 1}, {5, 3}}, data1, PREFIX_ROW_MAJOR);
    dpd_varray_view<double> v2(0, 2, {{1, 1}, {6, 3}, {2, 4}}, data2, PREFIX_COLUMN_MAJOR);
